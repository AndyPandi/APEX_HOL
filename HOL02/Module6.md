...
현재 프로젝트 레코드를 관리할 방법이 없습니다. 따라서 **Create Page 도구**를 사용하여 **SAMPLE$PROJECTS** 테이블에 양식 페이지를 작성하십시오.
...
7. Table/ View Name 항목에서 **SAMPLE$PROJECTS (table)** 선택.
   **``Next``** 버튼 클릭.
...
**STATUS_ID** 항목은 **SAMPLE$PROJECT_STATUS** 테이블의 외래키 입니다. 따라서 사용자가 숫자를 입력하도록 하기보다는 테이블 데이터를 기반으로 값 리스트로 데이터를 입력하도록 합니다.  Name, Project Lead 필드는 Text Field 로 업데이트 해야합니다. 마지막으로 audit 컬럼은 숨겨야 합니다.
...
2. 페이지 10의 Page Designer Rendering tree (왼쪽창) 에서 **P10_STATUS_ID** 클릭.
   ...
   *( 참고 : code 컬럼은 사용자에게 표시도는 값이고 id컬럼은 테이블의 컬럼으로 반환되는 값입니다. 테이블에는 상태가 올바른 순서로 표시되기 위해 display_order 컬럼이 표함되어 있습니다. )*
   ...
   - Display Extra Values : **Disable** 클릭
